# ============================================================
# EIA860_Utility.ps1 - Load Schedule 1 Utility Data  v2.3
# ============================================================
param(
...
try {
    ...
    foreach ($row in $data) {
        if (-not (Is-ValidRow $row 'Utility ID')) { continue }
        $dr = $dt.NewRow()
    ...
    Write-Host "  Read $($dt.Rows.Count) valid rows" -ForegroundColor Gray
