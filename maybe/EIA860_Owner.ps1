...
if(-not $ownerFile){Write-Warning "Owner file not found";Write-EIALog -conn $conn -logId $logId -status "Skipped" -reportYear $ReportYear -errorMessage "Owner file not found" -startTime $startTime;$conn.Close();exit 0}
Write-Host "Found: $($ownerFile.Name)" -ForegroundColor Green
$availableSheets=Get-ExcelSheetNames $ownerFile.FullName
$ownerTab=Find-Tab $availableSheets "Ownership"
...
try{
    ...
    if($null -eq $data -or $data.Count -eq 0){Write-Warning "No data in Owner tab";Write-EIALog -conn $conn -logId $logId -status "Skipped" -reportYear $ReportYear -errorMessage "No data in Owner tab" -startTime $startTime;$conn.Close();exit 0}
...
}catch{Write-EIALog -conn $conn -logId $logId -status "Failed" -reportYear $ReportYear -errorMessage "Owner read error: $_" -startTime $startTime;Write-Error $_;$conn.Close();exit 1}
Load-Staging $conn $dt "EIA.EIA860_OwnerData_Staging"
$result=Invoke-MergeSP $conn "EIA.usp_MergeEIA860OwnerData"
Write-EIALog -conn $conn -logId $logId -status "Success" -reportYear $ReportYear -tableName "EIA860_OwnerData" -rowsInserted $result.RowsInserted -rowsUpdated $result.RowsUpdated -rowsInFile $cnt -totalRows $result.TotalRows -tabsProcessed $ownerTab -startTime $startTime
Write-TabSummary "Owner" $cnt $result.RowsInserted $result.RowsUpdated $result.TotalRows ([int](New-TimeSpan -Start $startTime -End (Get-Date)).TotalSeconds) @($ownerTab)
$conn.Close()
