# ============================================================
# EIA-860 Schedule 3.5 MultiFuel Data -> SQL Server ETL
...
$stagingTable  = "EIA.EIA860_MultiFuelData_Staging"
$storedProc    = "EIA.usp_MergeEIA860MultiFuelData"
...
Write-Host "===== EIA-860 MultiFuel ETL - Year: $ReportYear =====" -ForegroundColor Cyan
...
$mfFile = Get-ChildItem -Path $extractDir -Recurse -Filter "*.xlsx" |
          Where-Object { $_.Name -like "3_5_*uel*" } | Select-Object -First 1
if (-not $mfFile) { Write-Warning "MultiFuel file not found - skipping"; $connection.Close(); exit 0 }
Write-Host "Found: $($mfFile.Name)" -ForegroundColor Green

$pkg = Open-ExcelPackage -Path $mfFile.FullName
...
@("ReportYear","UtilityId","UtilityName","PlantCode","PlantName","State",
  "GeneratorId","FuelSwitchCapable","CofireEnergySource1","CofireEnergySource2",
  "CofireEnergySource3","StatusTab") | ForEach-Object {
...
foreach ($tabSearch in @("Operable","Proposed","Retired")) {
    ...
    try { $excelData = Import-Excel -Path $mfFile.FullName -WorksheetName $matchedSheet -StartRow 2 -ErrorAction Stop }
    ...
    foreach ($row in $excelData) {
        ...
        $dr["FuelSwitchCapable"]   = [string]$row.'Switch When Operating?'
        $dr["CofireEnergySource1"] = [string]$row.'Cofire Energy Source 1'
        $dr["CofireEnergySource2"] = [string]$row.'Cofire Energy Source 2'
        $dr["CofireEnergySource3"] = [string]$row.'Cofire Energy Source 3'
...
Write-Host "`n MultiFuel: $($tabsLoaded -join ', ') | File: $totalCount | +$ri ~$ru | Total: $tr | ${dur}s" -ForegroundColor Green
$connection.Close()
