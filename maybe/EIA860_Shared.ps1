...
function Write-EIALog {
    ...
    try {
        ...
        # Null-safe: double-cast every string to guarantee non-null before .Replace()
        ...
        $tabsProcessed = [string]$tabsProcessed
        ...
        Write-Warning "Write-EIALog failed: $_"
        return $logId
    }
}
...
function Load-Staging {
    ...
    try {
        ...
        Write-Warning "TRUNCATE $stagingTable failed: $_"
    }
    if($null -eq $dataTable -or $dataTable.Rows.Count -eq 0){Write-Host "  No rows to load for $stagingTable" -ForegroundColor Yellow;return}
    try {
        ...
        Write-Warning "Bulk load to $stagingTable failed: $_"
    }
}
...
function Invoke-MergeSP {
    ...
    try {
        ...
        if($null -ne $reader){
            if($reader.Read()){
                $ri=[int]$reader["RowsInserted"]
                $ru=[int]$reader["RowsUpdated"]
                $tr=[int]$reader["TotalRowsInTable"]
            }
            $reader.Close()
        }
    } catch {
...
function Get-ExcelSheetNames {
    ...
    try {
        ...
        if($null -eq $pkg -or $null -eq $pkg.Workbook){Write-Warning "Could not open workbook: $filePath";return @()}
        $sheets=$pkg.Workbook.Worksheets|ForEach-Object{$_.Name}
        Close-ExcelPackage $pkg
        if($null -eq $sheets){return @()}
        return $sheets
    } catch {Write-Warning "Could not read sheet names: $_";return @()}
}

# ============================================================
# Find-Tab: Flexible sheet-name matching
# Tries exact, trimmed, wildcard, then keyword fallback
# ============================================================
function Find-Tab {
    ...
    if($match){return $match}
    # 4. For "Retired and Canceled", try matching just "Retired"
...
function Find-EIAFile {
    ...
    # Search recursively in case ZIP extracts into a subfolder
    return Get-ChildItem $extractPath -Recurse|Where-Object{$_.Name -like $pattern}|Select-Object -First 1
}
...
function Write-TabSummary {
    ...
    if($null -ne $tabsLoaded -and $tabsLoaded.Count -gt 0){Write-Host " Tabs: $($tabsLoaded -join ', ')" -ForegroundColor White}
...
Write-Host "EIA860_Shared.ps1 v2.6 loaded." -ForegroundColor Gray
