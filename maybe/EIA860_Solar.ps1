# ============================================================
# EIA-860 Schedule 3.3 Solar Data -> SQL Server ETL
...
$stagingTable  = "EIA.EIA860_SolarData_Staging"
$storedProc    = "EIA.usp_MergeEIA860SolarData"
...
Write-Host "===== EIA-860 Solar ETL - Year: $ReportYear =====" -ForegroundColor Cyan
...
$solarFile = Get-ChildItem -Path $extractDir -Recurse -Filter "*.xlsx" |
             Where-Object { $_.Name -like "3_3_*olar*" } | Select-Object -First 1
if (-not $solarFile) { Write-Warning "Solar file not found - skipping"; $connection.Close(); exit 0 }
Write-Host "Found: $($solarFile.Name)" -ForegroundColor Green

$pkg = Open-ExcelPackage -Path $solarFile.FullName
...
@("ReportYear","UtilityId","UtilityName","PlantCode","PlantName","State",
  "GeneratorId","SingleAxisTracking","DualAxisTracking","FixedTilt",
  "DCNetCapacity","TiltAngle","AzimuthAngle","StatusTab") | ForEach-Object {
...
foreach ($tabSearch in @("Operable","Retired")) {
    ...
    try { $excelData = Import-Excel -Path $solarFile.FullName -WorksheetName $matchedSheet -StartRow 2 -ErrorAction Stop }
    ...
    foreach ($row in $excelData) {
        ...
        $dr["SingleAxisTracking"] = [string]$row.'Single-Axis Tracking?'
        $dr["DualAxisTracking"]   = [string]$row.'Dual-Axis Tracking?'
        $dr["FixedTilt"]          = [string]$row.'Fixed Tilt?'
        $dr["DCNetCapacity"]      = [string]$row.'DC Net Capacity (MW)'
        $dr["TiltAngle"]          = [string]$row.'Tilt Angle'
        $dr["AzimuthAngle"]       = [string]$row.'Azimuth Angle'
...
Write-Host "`n Solar: $($tabsLoaded -join ', ') | File: $totalCount | +$ri ~$ru | Total: $tr | ${dur}s" -ForegroundColor Green
$connection.Close()
