# ============================================================
# EIA860_Shared.ps1
# Version 2.4 - Final null fix + updated column names
# ============================================================
...
function Write-EIALog {
    ...
    if ($logId -eq 0) {
        ...
        $sql = "UPDATE EIA.EIA860_LoadLog SET
                Status='$status',RowsInserted=$rowsInserted,
                RowsUpdated=$rowsUpdated,RowsInFile=$rowsInFile,
                TotalRowsInTable=$totalRows,TabsProcessed='$safeTabs',
                ErrorMessage='$errorMessage',DurationSeconds=$duration
                WHERE LogId=$logId;"
        $cmd = New-Object System.Data.SqlClient.SqlCommand($sql, $conn)
...
# ============================================================
# Get-Val: Safe cell value extraction
# Handles null, DBNull, empty PSCustomObject properties
# ============================================================
function Get-Val {
    param($row, [string]$colName)
    if ($null -eq $row)     { return "" }
    if ($null -eq $colName) { return "" }
    ...
    try {
        ...
        $str = [string]$val
        if ($str.Trim() -eq "" -or $str -eq " ")     { return "" }
        return $str.Trim()
    } catch { return "" }
}

# ============================================================
# Is-ValidRow: Check if a row has real data
# ImportExcel returns empty PSCustomObjects for blank rows
# We check if the key column has an actual non-empty value
# ============================================================
function Is-ValidRow {
    param($row, [string]$keyColumn)
    if ($null -eq $row) { return $false }
    try {
        # Check if the object even has this property
        $props = $row.PSObject.Properties
        if ($null -eq $props)          { return $false }
        $prop = $props[$keyColumn]
        if ($null -eq $prop)           { return $false }
        $val = $prop.Value
        if ($null -eq $val)            { return $false }
        if ($val -is [System.DBNull])  { return $false }
        $str = [string]$val
        if ($str.Trim() -eq "")        { return $false }
        return $true
    } catch { return $false }
}
...
function Invoke-MergeSP {
    ...
    $reader.Close()
    return @{ RowsInserted = $rowsInserted; RowsUpdated = $rowsUpdated; TotalRows = $totalRows }
}
...
function Get-EIAZipFile {
    ...
    if ($ManualMode) {
        ...
        try {
            ...
            if ($fileSize -lt 1MB) { Write-Error "ZIP too small - likely blocked"; return $false }
...
function Get-ExcelSheetNames {
    ...
    try {
        ...
        return $sheets
    } catch { Write-Warning "Could not read sheet names from: $filePath"; return @() }
}
