# ============================================================
# EIA860_MultiFuel.ps1 - Load Schedule 3.5 MultiFuel  v2.3
...
if (-not $multiFuelFile) { Write-Warning "MultiFuel file not found - skipping"; Write-EIALog -conn $conn -logId $logId -status "Skipped" -reportYear $ReportYear -errorMessage "MultiFuel file not found" -startTime $startTime; $conn.Close(); exit 0 }
...
foreach ($tab in @("Operable","Proposed","Retired and Canceled")) {
    ...
    try {
        ...
        $cnt = 0
        foreach ($row in $data) {
            if (-not (Is-ValidRow $row 'Plant Code')) { continue }
            $dr = $dt.NewRow()
        ...
        $tabsLoaded += "$actualTab($cnt)"; Write-Host "  $actualTab: $cnt rows" -ForegroundColor Gray
