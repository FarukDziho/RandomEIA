...
# Tab: 'Plant'
# Key fixes:
#   'Water Source' -> 'Name of Water Source'
#   'Primary Purpose' -> 'Primary Purpose (NAICS Code)'
# ============================================================
param(
...
foreach ($row in $data) {
    if ($null -eq $row -or (Get-Val $row 'Plant Name') -eq "") { continue }
    $dr                       = $dt.NewRow()
    ...
    $dr["WaterSource"]        = Get-Val $row 'Name of Water Source'           # Fixed
    $dr["PrimaryPurpose"]     = Get-Val $row 'Primary Purpose (NAICS Code)'   # Fixed
    $dt.Rows.Add($dr)
}
