...
# Key fixes:
#   'Turbine Hub Height (Meters)' -> 'Turbine Hub Height (Feet)'
#   'Turbine Rated Capacity (MW)' -> does not exist, removed
#   'Rotor Diameter (Meters)' -> does not exist, removed
#   'Design Wind Speed (mph)' added
# ============================================================
param(
...
$dt = New-DataTable @("ReportYear","UtilityId","UtilityName","PlantCode","PlantName",
      ...
      "HubHeight","DesignWindSpeed","WindQualityClass","StatusTab")
...
foreach ($tab in $tabsToLoad) {
    ...
    try {
        ...
        foreach ($row in $data) {
            ...
            $dr["HubHeight"]             = Get-Val $row 'Turbine Hub Height (Feet)'   # Fixed - Feet not Meters
            $dr["DesignWindSpeed"]       = Get-Val $row 'Design Wind Speed (mph)'     # Added
